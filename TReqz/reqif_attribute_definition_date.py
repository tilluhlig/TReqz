...
class reqif_attribute_definition_date(TReqz.reqif_attribute_definition):
    ...
    def decode(self, content: Element, id_dict: TReqz.reqif_id_dict = None):
        ...
        self.default_value = TReqz.reqif_utils.generate_object_by_element_class(
            content, id_dict, "./{0}DEFAULT-VALUE/{0}ATTRIBUTE-VALUE-DATE".format(namespace), "reqif_attribute_value_date")

        self.type = TReqz.reqif_utils.get_local_ref_from_element_text(
            content, id_dict, "./{0}TYPE/{0}DATATYPE-DEFINITION-DATE-REF".format(namespace))
